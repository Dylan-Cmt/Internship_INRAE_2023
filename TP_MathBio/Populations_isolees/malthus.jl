...
problem = ODEProblem(model_malthus, etat0, tspan, params_malthus)
...
begin
	plot(solution, label = "\$x(t)\$", lw = 2, title = "Modèle de Malthus\n n=$n, m=$m",xlabel = "time \$ t\$",ylabel = "Densité de population \$ x(t)\$",legend= :topleft)
end

# ╔═╡ 6cd65773-d052-485e-9281-046fb69c3f09
begin
	plot(solution,
		linewidth=3, xlabel="t", yaxis="x(t)", label="numerical", size=(500, 300))
		
	plot!(t -> x0 * exp((n-m)*t), lw=3, ls=:dash, label="exact")
