...
begin
	using PlutoUI
	using Plots
	using DifferentialEquations # pour ODEProblem et solve
	using StaticArrays # pour SVector
end
...
# ╔═╡ 9dc0a317-acfe-42de-a27f-f5f28977fc75
md"""
### Modèle de Malthus
...
# ╔═╡ fbdbdd2c-7b0c-40e1-be8a-fed3a9c62ff8
"""
    malthus(u, p, t)

Returns `du` for the Malthus model.
"""
...
# ╔═╡ 26442656-5108-4679-9c20-a14e7ddfeb52
md"""
### Modèle Logistique

$$\dot{x}(t) = rx(1-\frac{x}{K})$$

avec $r$ le taux de croissance intrinsèque de la population et $K$ la capacité de charge de l’environnement.
"""

# ╔═╡ df5abd5a-d483-4c18-865d-bff321cb50aa
"""
    logistic(u, p, t)

Returns `du` for the Logistic model.
"""
...
# ╔═╡ bda6a7a0-5fe8-45ff-81af-9092d62004b3
md"""
### Effet Allee

$$\dot{x}(t) = rx(1-\frac{x}{K})(\frac{x}{\epsilon}-1)$$

avec $r$ le taux de croissance intrinsèque de la population (par analogie avec la logistique), $K$ la capcité de charge de l’environnement et $ϵ$ le seuil en dessous duquel la population n’est pas viable (‘seuil de Allee’). On procède comme ci-dessus.
"""

# ╔═╡ ce185a8e-006b-4740-9d11-20956d24ae7b
"""
    allee(u, p, t)

Returns `du` for the Allee model.
"""
...
# ╔═╡ 8110127e-ee8c-42c8-8b29-0c17d9c5befd
md"""
### Généralisation
"""

# ╔═╡ a8368284-167b-4f17-8d84-b0060a1b291c
"""
    Modeling(etat0, p,tspan,f)

Create a type Modeling that contains the informations of an ODE.

# Example
```julia-repl
julia> etat0 = @SVector [0.1]
julia> Modeling(etat0,[3,2],(0.0,20.0),fct)
```
"""
struct Modeling
	etat0::SVector{1, Float64}
	p::Vector{Float64}
...
# ╔═╡ 4816c49c-98b6-4530-8746-7fbfd435bd3f
begin
	# initial density of population
	etat0 = @SVector [0.1]
	# modeling
	mod_malthus = Modeling(etat0,[3,2],(0.0,20.0),0.01,malthus)
	mod_logistic = Modeling(etat0,[1,10],(0.0,10.0),0.01,logistic)
	mod_allee = Modeling(etat0,[1,10,2],(0.0,3.0),0.01,allee)
end

# ╔═╡ e7ca8764-2f50-4f6b-99a0-f1d543ec9308
"""
    ode_solver(mod)

Set up the problem and returns the ODE solved.
"""
...
# ╔═╡ 9fd5abdc-bbd6-4433-92b5-a2909dc088b3
@doc """
    plot(mod::Modeling)

When applied to `Modeling` Type, plot the solution associated with the EDO.

# Example
```julia-repl
julia> plot(mod_malthus)
```
"""
function Plots.plot(mod::Modeling;kwargs...)
	plot(ode_solver(mod);kwargs...)
end

# ╔═╡ 221f3f3f-a5e1-4c6c-8d48-85f113368515
md"""
### Application
"""

# ╔═╡ a6e21b9c-ac93-4615-81d5-7544ff516c10
begin
	plot(mod_malthus
		,xlabel = "Temps \$t\$"
		,ylabel = "Densité de population \$x\$"
		,label="\$x(t)\$ numérique")
	title!("Simulation du modèle de Malthus ")
end

# ╔═╡ fb7cc071-f17f-4060-89bc-6e5cf1df45c6
begin
	plot(mod_logistic
		,xlabel = "Temps \$t\$"
		,ylabel = "Densité de population \$x\$"
		,label="\$x(t)\$ numérique")
	title!("Simulation du modèle Logistique ")
end

# ╔═╡ 0cf4e9ea-bf46-466c-aabc-46432db5f80a
begin
	plot(mod_allee
		,xlabel = "Temps \$t\$"
		,ylabel = "Densité de population \$x\$"
		,label="\$x(t)\$ numérique")
	title!("Simulation de l'effet Allee ")
end
