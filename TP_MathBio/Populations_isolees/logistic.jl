...
problem = ODEProblem(model_logistic, etat0, tspan, params_logistic)
...
begin
	...
	plot(solution, label = "\$x(t)\$", lw = 2, title = "Logistic Modeling \n r=$r, K=$K",xlabel = "time \$ t\$",ylabel = "Densité de population \$ x(t)\$",legend= :topleft)


	# équilibres
	plot!(solution.t, zero(1:length(solution.t)),linestyle =  :dashdot,label = "équilibre instable")
	plot!(solution.t, ones(length(solution.t))*K,linestyle =  :dashdot,label = "équilibre stable")
