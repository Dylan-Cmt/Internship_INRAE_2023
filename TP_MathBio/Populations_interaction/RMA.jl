...
# ╔═╡ f2ec946f-d691-4bd6-b857-7c1480464148
begin
	using PlutoUI
	using Plots
...
# ╔═╡ dd16cd5a-ee60-11ed-315a-21ef3186b7f5
TableOfContents()

# ╔═╡ c21a2c0c-4ffe-412c-8845-624887091562
md"""
# Dynamique de populations sous Julia
"""

# ╔═╡ 5477124f-3ea7-4b29-93e6-20fac132761f
md"""
## Populations en intéraction
"""

# ╔═╡ c3a7d15d-1d1d-4f21-8b85-33a9177e87ee
md"""
### Modèle de Rosenzweig-MacArthur

$$\begin{cases} \dot{x}(t) = rx(1 - \frac{x}{K}) - c \frac{x}{x +h} y \\ \dot{y}(t) = b \frac{x}{x +h} y - my \end{cases}$$

"""

# ╔═╡ 8f6f3e36-17cc-4faa-a9f7-0bded7ae88d2
md"""
#### Dynamique
"""

# ╔═╡ 2d5f17cb-3519-43e3-ba78-a406a174576c
begin
	...
	# specifics parameters  of the birds population
	r = 1.0
	K = 10.0
	c = 1.0
	h = 2.0
	...
	m = 1.0
	#encapsulation
	params_RMA = [r, K, c, h, b, m]
end

# ╔═╡ db0fa367-da00-48a8-bb01-7ce4610ab160
...
# ╔═╡ 2d82618a-eacc-436f-bdd7-dd15c1bacba3
problem = ODEProblem(model_RMA, etat0, tspan, params_RMA, saveat = pas_t)

# ╔═╡ 5990a4d1-9f78-4d42-bd22-a972f2c7699a
solution = solve(problem)

# ╔═╡ 4cde96f4-ec15-4666-a58c-17b4d5f9d8f1
begin
	plot(solution, xlabel = "temps", ylabel = "densité de population")
	title!("Dynamique du modèle de Rosenzweig MacArthur")
end

# ╔═╡ d7f4ccd1-4de6-4f40-a5ce-1ee37e6443fd
md"""
#### Plan de phase
"""

# ╔═╡ 6c3342c6-e26f-40e4-8b5b-c0ca3c53d172
begin
	...
	null_x_x = zero(yplot)       			 		# x = 0 isocline nulle de xdot
	null_x_y = (r/c)*(h .+ xplot) .* (1 .- xplot/K) # isocline nulle de xdot
	
	## isoclines nulles de ydot
	null_y_y = zero(xplot)        			 		# y = 0 isocline nulle de ydot
	null_y_x = ones(1,size(yplot)[1]) * m*h /(b-m)  # isocline nulle de ydot
...
# ╔═╡ 05d6711a-d6ed-4cff-bb44-748cb0c83d75
begin
	
	# isoclines nulles 
	plot(null_x_x, yplot, color = "green", label = "isoclines nulles de \$ dx \$")
	plot!(xplot, null_x_y, color = "green",label = false)
	plot!(xplot, null_y_y, color = "orange",label = "isoclines nulles de \$ dy \$")
	plot!(null_y_x, yplot, color = "orange",label = false)

	# équilibres
	scatter!([eq_extinct[1]], [eq_extinct[2]], color = "red", markersize = 5,label = false)
	scatter!([eq_coex[1]], [eq_coex[2]], color = "red", markersize = 5,label = false)
	scatter!([eq_prey[1]], [eq_prey[2]], color = "red", markersize = 5,label = false)

	# trajectoires
	plot!(solution, vars=(1, 2), xlims=(-0.1, 11.0), size=(500, 300),label = "trajectoire", color = "blue")
	
	# enluminures
	xlabel!("Proies \$x\$")
	ylabel!("Prédateurs \$y\$")

	title!("Dynamiques Proies - Prédateurs modèle de Rosenzweig MacArthur",titlefont = font(9))
end

# ╔═╡ 37c3f346-97b3-4306-a41e-3dcf122221c3
begin
	
	# les deux fonctions
	der_x(x, y) = model_RMA([x y], params_RMA, 0)[1]
	der_y(x, y) = model_RMA([x y], params_RMA, 0)[2]
	
	# un vecteur x et un vecteur y transposé
	x = LinRange(0.0, 8.0, 10)
	y = x
	
	# qui sont copiés collés suffisamment de fois dans les dimensions de l'autre
	X = x'.*(ones(length(y)))
	Y = ones(length(x))'.*y
...
# ╔═╡ 69550464-b1ce-40b2-abe2-4ce9b368cf30
quiver!(X, Y, quiver=(u, v), width = .02,lw = 1)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "b2b60699de0963c4d9915240414337d3ade7ddda"
