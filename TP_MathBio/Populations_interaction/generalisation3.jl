...
# ╔═╡ 53ed9a1b-e9e0-4918-9957-0c1b7407c5bf
md"""
...
# ╔═╡ 2b1629b9-8388-498d-9d7f-069d6cb54af8
"""
    lv(u, p, t)

Returns a vector `du` containing `dx` and `dy` for the Lotka-Volterra model.
"""
...
Returns a vector `du` containing `dx` and `dy` for the Rosenzweig-MacArthur model.
"""
...
# ╔═╡ 205d8209-3c30-4ada-9b15-0b3a059ca055
"""
    Modeling(etat0, p,tspan,step,fct)

Create a type Modeling that contains the informations of an ODE.

# Example
```julia-repl
julia> etat0 = @SVector [0.1,1.0]
julia> Modeling(etat0,[3,2,1],(0.0,20.0),0.1,rma)
```
"""
struct Modeling
...
# ╔═╡ 229bd9de-0dca-4b44-855d-2532fed16f8f
"""
    ode_solver(mod)

Set up the problem and returns the ODE solved in a data frame.
# Example
```julia-repl
julia> ode_solver(lv,model_LV)
```
"""
function ode_solver(mod::Modeling)
	mod_prob = ODEProblem(mod.f,mod.etat0,mod.tspan, mod.p,saveat = mod.pas)
	mod_sol = solve(mod_prob)
	df = DataFrame(mod_sol)
	rename!(df, :timestamp => :time, :value1 => :x, :value2 => :y);
	return df
end

# ╔═╡ 1568bc96-9418-46f1-a568-6630305a5956
"""
    dynamic(mod, f_mod)
...
# Example
```julia-repl
julia> dynamic(lv,model_LV)
```
"""
function dynamic(mod::Modeling)
	df = ode_solver(mod)
	
	fig1, ax1 = plt.subplots()
	
	ax1.plot(df.time, df.x, color = "C0", label ="proies \$x\$")
	ax1.plot(df.time, df.y, color ="C1", label = "prédateurs \$y\$")
	ax1.set_xlabel("temps")
	ax1.set_ylabel("densités de population")
	ax1.grid()
	ax1.legend()
	PyPlot.gcf() # affiche le plot
end

# ╔═╡ 86d9947c-36a3-4de1-a551-80fa2e8d55cb
md"""
### Application
"""
...
begin
	etat0 = @SVector [1.0, 2.5]
	mod_rma = Modeling(etat0,[1.0, 10.0, 1.0, 2.0, 2.0, 1.0],(0.0,80.0),0.01,rma)
	mod_lv = Modeling(etat0,[1.0, 1.0, 1.0, 1.0],(0.0,30.0),0.01,lv)
end

# ╔═╡ c99f1164-2525-46ef-bb58-c01bdd5be22b
dynamic(mod_rma)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "ffeefefe6a5ec5e53ca227a75a84bf3f821d7811"
