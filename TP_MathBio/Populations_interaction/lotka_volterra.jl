...
$$\begin{cases} \dot{x}(t) = rx - cxy \\ \dot{y}(t) = bxy - my \end{cases}$$
...
problem = ODEProblem(model_LV, etat0, tspan, params_LV, saveat = pas_t)
...
begin
	...
	null_x_y = (zero(xplot).+1) .*(r/c)   	 # y = r/c isocline nulle de xdot
	
	## isoclines nulles de ydot
	null_y_y = zero(xplot)        			 # y = 0 isocline nulle de ydot
	null_y_x = (zero(yplot).+1) .*(m/b)   	 # x = m/b isocline nulle de ydot
...
begin
	...
	## isoclines nulles 
	plot(null_x_x, yplot, color = "green")
	plot!(xplot, null_x_y, color = "green")
	plot!(xplot, null_y_y, color = "orange")
	plot!(null_y_x, yplot, color = "orange")

	## équilibres
	scatter!([eq_extinct[1]], [eq_extinct[2]], color = "red", label = "", markersize = 5)
	scatter!([eq_coex[1]], [eq_coex[2]], color = "blue", label = "", markersize = 5)

	# trajectoires
	plot!(solution, vars=(1, 2), xlims=(-0.1, 3.0), size=(500, 300))
	
	## enluminures
	xlabel!("Proies \$x\$")
	ylabel!("Prédateurs \$y\$")

	title!("Dynamiques Proies - Prédateurs modèle de Lotka Volterra",titlefont = font(10))
end
...
begin
	...
	# les deux fonctions
	der_x(x, y) = model_LV([x y], params_LV, 0)[1]
	der_y(x, y) = model_LV([x y], params_LV, 0)[2]
	
	# un vecteur x et un vecteur y transposé
	x = LinRange(0.0, 2.2, 10)
	y = x
...
quiver!(X, Y, quiver=(u, v), width = .002)
